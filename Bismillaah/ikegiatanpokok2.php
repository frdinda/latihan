...
require 'db.php';
	...
	if ((!strlen(trim($kegiatanpokok))) || (!strlen(trim($definisikegiatan))) || (!strlen(trim($outputkegiatan))) || (!strlen(trim($karakteristikkegiatan))) || (!strlen(trim($lingkupkegiatan)))){
		...
		if($test == "sama"){
			echo "<script>alert('Kegiatan Pokok Sudah Pernah Diinput!'); window.location.replace('tambahkegiatanpokok.php');</script>";
		}else {
			...
			if($resultinsert === TRUE){
				...
				if($resultbobotnilai -> num_rows == 0){
					...
					if($resultupdateposisi === TRUE){
						...
						echo "<script>alert('Data Tidak Berhasil Diinput!'); window.location.replace('tambahkegiatanpokok.php');</script>";
					}
				}
			}
		}
