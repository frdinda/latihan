  ...
  <fieldset>
    ...
    <table border="1">
      ...
      <tr>
        ...
        <td><?= $data['alamat'];?></td>
        <td><a href="update_murid_5.php?id=<?= $data['nis'];?>">Update</a></td>
        <td><a href="latihan_crud_5.php?id=<?= $data['nis'];?>">Delete</a></td>
      </tr>
      <?php
        }
      ?>
    ...
    function tambahdata($conn){
      ...
      if(!empty($nis) && !empty($nama) && !empty($tanggal_lahir) && !empty($alamat)){
        ...
        if($conn->query($sql) === TRUE){
          header ('location: latihan_crud_5.php');
        }else{
