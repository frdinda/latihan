...
require 'db.php';
	...
	if ((!strlen(trim($kegiatanpokok))) || (!strlen(trim($definisikegiatan))) || (!strlen(trim($outputkegiatan))) || (!strlen(trim($karakteristikkegiatan))) || (!strlen(trim($lingkupkegiatan)))){
		...
		while($kegiatanunit = $resultselect -> fetch_assoc()){
			if($kegiatanpokok === $kegiatanunit['Kegiatan_Pokok']){
				$test = "sama";
			}else {
				$test = "";
			}
		...
		if($test == "sama"){
			echo "<script>alert('Kegiatan Pokok Sudah Pernah Diinput!'); window.location.replace('editkegiatanpokok.php');</script>";
		}else {
			$sqlupdate = "UPDATE `kegiatan_pokok` SET `Kegiatan_Pokok`='$kegiatanpokok',`Definisi_Kegiatan`='$definisikegiatan',`Output_Kegiatan`='$outputkegiatan',`Karakteristik_Kegiatan`='$karakteristikkegiatan',`Lingkup_Kegiatan`='$lingkupkegiatan' WHERE Kode_Unit = '$kodeunit' AND Kegiatan_Pokok = '$name'";
			...
			if($resultupdate === TRUE){
				...
				if($resultbobotnilai -> num_rows == 0){
					...
					if($resultupdateposisi === TRUE){
						echo "<script>alert('Data Berhasil Diinput!'); window.location.replace('editkegiatanpokok.php');</script>";
					} else{
						echo "<script>alert('Data Tidak Berhasil Diinput!'); window.location.replace('editkegiatanpokok.php');</script>";
					}
				}
			}
		}
